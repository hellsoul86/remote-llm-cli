...
test("shows target failure details instead of Done when async job status is succeeded but target failed", async ({ page }) => {
  let jobPollCount = 0;
  ...
  await page.route("**/v1/metrics", async (route) => {
    await route.fulfill({
      status: 200,
      json: {
        jobs: { total: 1, pending: 0, running: 0, succeeded: 0, failed: 1, canceled: 0, retry_attempts: 0 },
        queue: { depth: 0, workers_total: 3, workers_active: 0, worker_utilization: 0 },
        success_rate: 0
      }
    });
  ...
  await page.route("**/v1/jobs/job_3", async (route) => {
    jobPollCount += 1;
    if (jobPollCount < 2) {
      await route.fulfill({
        status: 200,
            ...
            targets: [
              {
                ...
                error_hint: "local command exited non-zero; inspect stderr/stdout"
              }
            ]
          }
        }
  ...
  await page.route("**/v1/jobs/job_3/events**", async (route) => {
    await route.fulfill({
      status: 200,
      json: {
        job_id: "job_3",
        ...
        next_after: 3,
        events: [
          { seq: 1, job_id: "job_3", type: "job.running", created_at: "2026-03-02T00:00:00Z" },
          { seq: 2, job_id: "job_3", type: "target.started", host_name: "local-default", attempt: 1, created_at: "2026-03-02T00:00:00Z" },
          { seq: 3, job_id: "job_3", type: "target.done", host_name: "local-default", status: "failed", exit_code: 1, created_at: "2026-03-02T00:00:00Z" }
        ]
      }
    });
  });
        ...
        jobs: [
          {
            ...
            prompt_preview: "smoke",
            queued_at: "2026-03-02T00:00:00Z",
            result_status: status === "succeeded" ? 502 : undefined,
            total_hosts: 1,
            succeeded_hosts: status === "succeeded" ? 0 : 0,
            failed_hosts: status === "succeeded" ? 1 : 0,
            fanout: 1,
            duration_ms: status === "succeeded" ? 1000 : 0
          }
        ]
